...
public class Main {
    public static void main(String[] args) {
        int[] numbers = new int[]{3, 5, 2, -4, 8, 11};
        twoSum(numbers, 7);
    }

    static List twoSum(int[] numbers, int sum) {
        int length = numbers.length;
        List sumArray = new ArrayList<ArrayList<Integer>>(length);
        Set values = new HashSet<Integer>() {
        };
        for (int i = 0; i <= numbers.length - 1; i++) {
            int remainder = sum - numbers[i];
            if (values.contains(remainder)) {
                sumArray.add(Arrays.asList(remainder, numbers[i]));
            }
            values.add(numbers[i]);
        }
        sumArray.stream().forEach(System.out::println);
        return sumArray;
    }
}
